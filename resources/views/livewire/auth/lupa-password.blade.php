...
<div class="flex min-h-screen items-center justify-center px-4 py-10">
    <div class="w-full max-w-md">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
            ...
            <p class="mt-1 text-sm text-slate-500">Masukkan email akun Anda — kami kirim tautan untuk mengatur ulang kata sandi.</p>

            @if ($status)
                <div class="mt-4 rounded-lg border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-700">{{ $status }}</div>
            @endif
            ...
            <form wire:submit="kirim" class="mt-6 space-y-5">
                ...
                <button type="submit" wire:loading.attr="disabled" wire:target="kirim"
                    ...
                    <svg wire:loading wire:target="kirim" class="h-4 w-4 animate-spin" viewBox="0 0 24 24" fill="none">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
                    </svg>
                    <span wire:loading.remove wire:target="kirim">Kirim tautan reset</span>
