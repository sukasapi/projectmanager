<div class="space-y-5">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-xl font-bold tracking-tight text-slate-900">Konfigurasi Pipeline</h1>
            <p class="text-sm text-slate-500">Atur tahapan produksi per fase. Tambah, ubah, atau nonaktifkan sesuai kebutuhan studio.</p>
        </div>
                            ...
                            <tr wire:key="tahap-{{ $t->id }}" class="transition hover:bg-slate-50 {{ $t->is_active ? '' : 'opacity-60' }}">
                                ...
                                <td class="px-4 py-2.5">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <button x-on:click="$confirm(@js('Hapus tahap “'.$t->name.'”? (dapat dipulihkan)'), { danger: true }).then(ok => ok && $wire.delete({{ $t->id }}))" class="inline-flex rounded-md bg-slate-100 p-1.5 text-red-500 transition hover:bg-red-100 hover:text-red-700" title="Hapus">
